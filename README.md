...
| 历史速率图 | `/admin` 上的趋势图：按模型堆叠的 token/请求速率，以及失败率与平均延迟两条「稳定性」曲线，可切 1 小时 / 6 小时 / 24 小时窗口 |
...
四种口径共用同一份数据（切换口径不重新请求）：

- **Token 速率 / 请求速率**：堆叠面积图，纵轴是**每分钟**的量。后端给的是桶内总量，前端按桶长归一化，所以切窗口时曲线量级可比。
- **失败率 / 平均延迟**：叠加折线（率不能堆叠）。失败率纵轴固定 0–100%，否则一次失败就把曲线顶满，看不出「本来是 0」。

图例可点掉单个模型；鼠标划过出该桶的明细。图下方是所选区间每个模型的请求数、失败数、失败率、token、平均与最大延迟，按失败率降序——**失败率从 0 开始爬、延迟抬升，就是上游开始不稳的信号**，比单看总量更早。

口径与后端实现：

- 历史是**进程内**的 24 小时分钟环形缓冲（每模型 1440 个槽），与「最近 60 秒 TPM/RPM」用的秒级环形桶并存：后者给精确的当前值，前者给长趋势。**重启清零**，不做落盘——统计本来就是内存态。
- 每个请求记入 token、请求数、失败数、延迟累计与单次最大延迟。
